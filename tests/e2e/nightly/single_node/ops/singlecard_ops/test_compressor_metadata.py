...
def test_device_metadata_external_events_graph_replay():
    torch.npu.set_device(0)
    executor = DeviceMetadataExecutor()
    persistent = torch.empty(8, dtype=torch.int32, device="npu")
    group_id = id(persistent)
    graphs = {}
    outputs = {}
    snapshots = []
    for size in (4, 8):
        descriptor = BatchDescriptor(num_tokens=size, num_reqs=size)
        output = torch.empty(size, dtype=torch.int32, device="npu")
        executor.submit(
            (DeviceMetadataTask(DeviceMetadataStage.ATTENTION, partial(persistent.fill_, size), group_id),),
            descriptor,
        )
        graph = torch.npu.NPUGraph()
        with torch.npu.graph(graph):
            executor.wait(DeviceMetadataStage.ATTENTION, group_id)
            output.copy_(persistent[:size])
        executor.release()
        graphs[size] = graph
        outputs[size] = output

    for size, value in ((4, 11), (8, 22), (4, 33), (8, 44)):
        executor.submit(
            (DeviceMetadataTask(DeviceMetadataStage.ATTENTION, partial(persistent.fill_, value), group_id),),
            BatchDescriptor(num_tokens=size, num_reqs=size),
        )
        graphs[size].replay()
        snapshots.append((outputs[size].clone(), value))
        executor.release()

    torch.npu.synchronize()
    for snapshot, value in snapshots:
        assert torch.all(snapshot.cpu() == value)
