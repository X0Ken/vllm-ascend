...
def test_device_metadata_requires_explicit_opt_in(monkeypatch, enabled, has_provider):
    class Provider:
        def __init__(self):
            self.enabled = False
    ...
    provider = Provider()
    builder = provider if has_provider else object()
    runner = NPUModelRunner.__new__(NPUModelRunner)
    runner.ascend_config = SimpleNamespace(async_dsv4_metadata=enabled)
    ...
    monkeypatch.setattr(model_runner_v1, "DeviceMetadataExecutor", make_executor)

    runner._initialize_device_metadata()

    active = enabled and has_provider
    assert make_executor.call_count == int(active)
    assert provider.enabled is active
    if active:
        assert runner.device_metadata_providers == {id(provider): provider}
        assert runner.device_metadata_executor is make_executor.return_value
    else:
        assert runner.device_metadata_providers is None
        assert runner.device_metadata_executor is None
